...
> `wsl --version` - посмотреть версию (или для старой версии `wsl --status`)
>
> `wsl --shutdown` - выгрузить виртуальную машину
>
> `wsl --uninstall` - удалить
>
...
В Linux для выполнения операции копирования из терминала в буфер обмена и наоборот используется пара утилит: `pbcopy` и `pbpaste`. Чтобы сохранить оригинальное поведение для WSL, создадим псевдонимы. Для этого добавим строки в файл настройки используемого шелла (у меня `zsh`) `~/.zshrc`:
